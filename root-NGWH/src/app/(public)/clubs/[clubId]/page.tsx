...
export default async function ClubProfilePage({ params }: ClubProfilePageProps) {
  ...
  let club: (ClubDetail & { is_approved?: boolean }) | null;
  try {
    club = await getClubDetailForView(
      numericId,
      session.user?.id,
  ...
  const u20Images = club.u20_athlete_images || [];

  return (
    <Container>
      <div className={styles.page}>
        {!club.is_approved && (
          <div style={{ background: "#78350f", color: "#fef3c7", border: "1px solid #d97706", padding: "0.85rem 1.25rem", borderRadius: "8px", marginBottom: "1rem", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "0.5rem" }}>
            <span>{t("clubs.profile.pendingNotice")}</span>
            {isOwner && (
              <Link href={`/account/clubs/${club.id}/edit`} className={styles.editClubBtn}>
                ✏️ {t("clubs.profile.editClubBtn")}
              </Link>
            )}
          </div>
        )}
        {isOwner && club.is_approved && (
          <div className={styles.ownerBanner}>
            <span>{t("clubs.profile.ownerBanner")}</span>
            <Link
        ...
        {(club.capability_profile || u20Images.length > 0 || club.u20_athlete_list) && (
          <section className={styles.documentsSection}>
            ...
            <div className={styles.documentGrid}>
              ...
              {u20Images.length > 0 ? (
                <div className={styles.documentCard} style={{ gridColumn: "1 / -1" }}>
                  <h3 className={styles.documentCardTitle}>
                    {t("clubs.profile.documents.u20AthleteList")}{" "}
                    {t("clubs.profile.documents.imagesCount", { count: u20Images.length })}
                  </h3>
                  <div
                    style={{
                      display: "grid",
                  ...
                  >
                    {u20Images.map((src, idx) => (
                      <a key={idx} href={src} target="_blank" rel="noreferrer">
                        <img
                          src={src}
                          alt={t("clubs.profile.documents.u20ImageAlt", { index: idx + 1 })}
                          style={{
                            width: "100%",
                            aspectRatio: "1",
                            objectFit: "cover",
