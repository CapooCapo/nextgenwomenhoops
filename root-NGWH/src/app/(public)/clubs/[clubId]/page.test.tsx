...
const mockedGetClubById = jest.mocked(getClubDetailForView);
...
describe("ClubProfilePage", () => {
  ...
  it("renders the club header and all confirmed sections for an approved club", async () => {
    mockedGetClubById.mockResolvedValueOnce({ ...fullClub, is_approved: true } as unknown as Awaited<ReturnType<typeof getClubDetailForView>>);

    render(await ClubProfilePage({ params: Promise.resolve({ clubId: "1" }) }));

    expect(mockedGetClubById).toHaveBeenCalledWith(1, undefined, false);
